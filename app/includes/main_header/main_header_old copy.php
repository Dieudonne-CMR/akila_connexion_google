    ...
    //   var_dump($matricule_ad);
      ...
      if($permission==2):
         $matricule_akila_blog = "1011121121"; //$_SESSION['skl']['matricule_akila_blog']; 
           ...
           if($active ==2){
            // var_dump($matricule_ad); recuper la premier entreprise d'un gerant
            ...
            //============= 01/07/2024 Nouvo ; Recuper la premier entreprises d'un proprietaire
            $data1 = $Register->query("SELECT * FROM website WHERE
               matricule_admin=:matricule LIMIT 1", array("matricule" => $matricule_ad)); // recupere toutes les entreprises que gere un administrateurs
           ...
           $data = ajouterElements($data, $data1);
         //========== fin de 01/07/2024 
      ...
      if (!empty(@$data)):
            $matricule_akila_blog=$data[0]->matricule_entreprise; // recupere le premier matricule de l'entreprise
