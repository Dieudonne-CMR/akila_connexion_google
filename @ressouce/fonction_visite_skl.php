<?php  
  ...
  $get_visit = $DB->db->query("SELECT * FROM `visiteurs_skl` WHERE date_visite = '$date_jour' AND id_website = '$idsite'");
  if ($get_visit->rowCount()<1) {
    ...
    $DB->db->query("INSERT INTO visiteurs_skl (id_website, date_visite, heure_visite, visiteurs, nbre) VALUES ('$idsite', '$date_jour', '$heure_visite', '$info', '1')");
      ...
      while ($data_visit = $get_visit->fetch()) {
          ...
          $DB->db->query("UPDATE visiteurs_skl SET visiteurs='$new_info', nbre='$nbre_visiteur', heure_visite='$heure_visite' WHERE date_visite = '$date_jour' AND id_website = '$idsite'");
          // echo$new_info;
      }
    }
?>